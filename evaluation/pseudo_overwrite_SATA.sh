...
do
    ...
    TARGET_FILENAME=T0
    CREAT_READ=1
    g++ -D RAND_DUMMY=0 -D CREAT_READ=$CREAT_READ -D OPT_DIRECT=$OPT_DIRECT -D TARGET_FILENAME=\"$TARGET_FILENAME\" -D TARGET_FOLDER=\"$TARGET_FOLDER\" -D WRITE_COUNT=$WRITE_COUNT -D FILE_SIZE=$FILE_SIZE -D APPEND_SIZE=$APPEND_SIZE -D DUMMYAPPEND_SIZE=$DUMMYAPPEND_SIZE -D READ_ENABLE=$READ_ENABLE -o $EXE_PATH $SRC_PATH

    ./"$EXE_PATH" >> "$RESULT_FOLDER"pseudo_overwrite_"$RESULT_NAME"_off.txt
...
do
    ...
    ./"$EXE_PATH" >> "$RESULT_FOLDER"pseudo_overwrite_"$RESULT_NAME"_on.txt
